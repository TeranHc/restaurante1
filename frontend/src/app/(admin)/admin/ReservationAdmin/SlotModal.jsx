...
const SlotModal = ({ slot, restaurants, selectedDate, selectedRestaurant, onSubmit, onClose, onDelete }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = slot
        ? `${process.env.NEXT_PUBLIC_API_URL}/available-slots/${slot.id}`
        : `${process.env.NEXT_PUBLIC_API_URL}/available-slots`;
  ...
  const handleDelete = async () => {
    ...
    try {
      // ✅ CORREGIDO: Usar variable de entorno en lugar de localhost hardcodeado
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/available-slots/${slot.id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' }
      });
